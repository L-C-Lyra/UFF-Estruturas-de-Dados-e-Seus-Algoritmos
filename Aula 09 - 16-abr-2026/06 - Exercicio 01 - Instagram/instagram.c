...
int seguidores(TGrafo *g, char *nome, int imprime) {
    ...
    int cont = 0;
    while (vertice != NULL) {
        if (busca_vizinho(vertice->prim_vizinho, nome) != NULL) {
            cont++;
            if (imprime) printf("%s ", vertice->nome);
        }
        vertice = vertice->prox;
    }
    if (imprime) printf("\n");
...
TGrafo *mais_popular(TGrafo *g) {
    ...
    while (vertice != NULL) {
        int num_seguidores = seguidores(g, vertice->nome, 0);
...
int segue_mais_velho(TGrafo *g, int imprime) {
    ...
    while (vertice != NULL) {
        if (vertice->prim_vizinho != NULL) {
            int mais_velho_apenas = 1;
            TVizinho *vizinho = vertice->prim_vizinho;
            while (vizinho != NULL) {
                TGrafo *vertice_vizinho = busca_vertice(g, vizinho->nome);
                if ((vertice_vizinho != NULL) && (vertice_vizinho->idade <= vertice->idade)) {
                    mais_velho_apenas = 0;
                    break;
                }
                vizinho = vizinho->prox;
            }
            if(mais_velho_apenas) {
                cont++;
                if (imprime) printf("%s ", vertice->nome);
            }
        }
