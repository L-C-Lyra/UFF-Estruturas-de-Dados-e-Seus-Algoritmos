...
int ehNaoOrientado(TGrafo *g) {
    ...
    while (vertice_atual != NULL) {
        int v1 = vertice_atual->id_vertice;
        TVizinho *vizinho_atual = vertice_atual->prim_vizinho;
        while (vizinho_atual != NULL) {
            int v2 = vizinho_atual->id_vizinho;
            int peso_ida = vizinho_atual->peso;
            TGrafo *vertice_proximo = busca_vertice(g, v2);
            if (vertice_proximo == NULL) return 0;
            TVizinho *aresta_volta = busca_vizinho(vertice_proximo->prim_vizinho, v1);
            if ((aresta_volta == NULL) || (aresta_volta->peso != peso_ida)) return 0;
