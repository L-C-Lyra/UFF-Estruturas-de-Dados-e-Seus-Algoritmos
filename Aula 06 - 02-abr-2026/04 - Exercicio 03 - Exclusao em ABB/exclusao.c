...
TNoA *exclui(TNoA *raiz, int chave) {
    ...
    if (chave < raiz->chave) raiz->esq = exclui(raiz->esq, chave);
    else if (chave > raiz->chave) raiz->dir = exclui(raiz->dir, chave);
    else {
        if (raiz->esq == NULL) {
            TNoA *tmp = raiz->dir;
            free(raiz);
        ...
        TNoA *maior_esq = raiz->esq;
        while (maior_esq->dir != NULL) maior_esq = maior_esq->dir;
