...
void conta_mesmos_nos(TNoA *nodo, int cont[], int val) {
    if (nodo != NULL) {
        cont[nodo->info] += val;
        conta_mesmos_nos(nodo->esq, cont, val);
        conta_mesmos_nos(nodo->dir, cont, val);
    }
}

int mesmos_nos(TNoA *a1, TNoA *a2) {
    int cont[256] = {0};
    conta_mesmos_nos(a1, cont, 1);
    conta_mesmos_nos(a2, cont, -1);
    for (int i = 0; i < 256; i++) {
